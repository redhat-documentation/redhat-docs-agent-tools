  ...
  def validate_yaml(content, step_num)
    begin
      ...
      if content.include?("apiVersion:")
        ...
        cli_tool = if system('which oc > /dev/null 2>&1')
                     'oc'
        ...
        if cli_tool
          Tempfile.open(['resource', '.yaml']) do |f|
            ...
            if status.success?
              puts "[VALID] Resource logic (dry-run via #{cli_tool}) passed for Step #{step_num}."
            else
              puts "[FAILURE] Resource validation failed: #{stderr}"
            end
          end
        else
          puts "[SKIP] No oc or kubectl CLI found — skipping resource dry-run for Step #{step_num}."
        end
      end
