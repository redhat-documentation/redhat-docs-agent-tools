...
| Procedure | Step-by-step instructions | Gerund phrase (verb + -ing) |
| Reference | Lookup data (tables, lists) | Noun phrase |
| Assembly | Combine modules into user story | Gerund if includes procedures |
...
- **If including actions**: Ensure the heading remains a noun phrase, NOT a gerund
...
1. **Title**: H1 heading with **gerund phrase** ("Configuring...", "Installing...", "Deploying...")
...
- [ ] Title uses gerund phrase (verb + -ing)
- [ ] Anchor ID includes `_{context}`
...
1. **Title**: Gerund form if includes procedures, noun phrase otherwise
...
- **With procedure modules**: Use gerund form (e.g., "Encrypting block devices using LUKS")
...
- [ ] Title matches content (gerund if procedures included)
...
| Procedure title | `= Database Configuration` | `= Configuring the database` |
